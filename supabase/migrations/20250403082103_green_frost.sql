/*
  # Create room status function and view

  1. Changes
    - Create function to get room status
    - Create view for room status
    - Add proper security and RLS

  2. Security
    - Function is SECURITY DEFINER
    - View inherits RLS from rooms and assets tables
    - Only authenticated users can access the view
*/

-- Drop existing function and view
DROP VIEW IF EXISTS room_status;
DROP FUNCTION IF EXISTS get_room_status(uuid);

-- Create function to get room status
CREATE FUNCTION get_room_status(room_id uuid)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
